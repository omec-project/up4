...
public class Up4TranslatorImpl implements Up4Translator {
  ...
  @Override
  public PacketDetectionRule fabricEntryToPdr(FlowRule entry) throws Up4TranslationException {
    ...
    if (TranslatorUtil.fieldIsPresent(match, SouthConstants.HDR_TEID)) {
      ...
      Ip4Address tunnelDst =
          TranslatorUtil.getFieldAddress(match, SouthConstants.HDR_TUNNEL_IPV4_DST);
      pdrBuilder.withTeid(teid).withTunnelDst(tunnelDst);
  ...
  @Override
  public PacketDetectionRule up4EntryToPdr(PiTableEntry entry) throws Up4TranslationException {
    var pdrBuilder = PacketDetectionRule.builder();
    ...
    if (srcInterface == NorthConstants.IFACE_ACCESS) {
      ...
      pdrBuilder.withTunnel(
          TranslatorUtil.getFieldValue(entry, NorthConstants.TEID_KEY),
    ...
    // Now get the action parameters, if they are present (entries from delete writes don't have
    // parameters)
    PiAction action = (PiAction) entry.action();
    PiActionId actionId = action.id();
    if (actionId.equals(NorthConstants.LOAD_PDR) && !action.parameters().isEmpty()) {
      ImmutableByteSequence sessionId =
          TranslatorUtil.getParamValue(entry, NorthConstants.SESSION_ID_PARAM);
  ...
  @Override
  public ForwardingActionRule fabricEntryToFar(FlowRule entry) throws Up4TranslationException {
    ...
    // Match keys
    farBuilder.withSessionId(farId.getPfcpSessionId()).setFarId(farId.getSessionLocalId());
    ...
    if (actionId.equals(SouthConstants.FABRIC_INGRESS_SPGW_LOAD_TUNNEL_FAR)
      ...
      short tunnelSrcPort =
          (short) TranslatorUtil.getParamInt(action, SouthConstants.TUNNEL_SRC_PORT);
  ...
  @Override
  public ForwardingActionRule up4EntryToFar(PiTableEntry entry) throws Up4TranslationException {
    // First get the match keys
    ImmutableByteSequence sessionId =
        TranslatorUtil.getFieldValue(entry, NorthConstants.SESSION_ID_KEY);
    int localFarId = TranslatorUtil.getFieldInt(entry, NorthConstants.FAR_ID_KEY);
    var farBuilder = ForwardingActionRule.builder().setFarId(localFarId).withSessionId(sessionId);
    ...
    if (!action.parameters().isEmpty()) {
      ...
      farBuilder
          .setDropFlag(TranslatorUtil.getParamInt(entry, NorthConstants.DROP_FLAG) > 0)
  ...
  @Override
  public UpfInterface fabricEntryToInterface(FlowRule entry) throws Up4TranslationException {
    Pair<PiCriterion, PiTableAction> matchActionPair = TranslatorUtil.fabricEntryToPiPair(entry);
    PiCriterion match = matchActionPair.getLeft();
    PiAction action = (PiAction) matchActionPair.getRight();
  ...
  @Override
  public FlowRule farToFabricEntry(
      ForwardingActionRule far, DeviceId deviceId, ApplicationId appId, int priority)
    ...
    if (!far.encaps()) {
      ...
      if (far.tunnelSrc() == null
          || far.tunnelDst() == null
          || far.teid() == null
          || far.tunnel().srcPort() == null) {
        throw new Up4TranslationException(
            "Not all action parameters present when translating "
                + "intermediate encapsulating/buffering FAR to physical FAR!");
      }
      // TODO: copy tunnel destination port from logical switch write requests, instead of
      // hardcoding 2152
      PiActionId actionId =
          far.buffers()
              ? SouthConstants.FABRIC_INGRESS_SPGW_LOAD_DBUF_FAR
  ...
  @Override
  public FlowRule pdrToFabricEntry(
      PacketDetectionRule pdr, DeviceId deviceId, ApplicationId appId, int priority)
      throws Up4TranslationException {
    PiCriterion match;
    PiTableId tableId;
    if (pdr.matchesEncapped()) {
      ...
      throw new Up4TranslationException(
          "Flexible PDRs not yet supported! Cannot translate " + pdr.toString());
    }
        ...
        PiAction.builder()
            ...
            .withParameters(
                Arrays.asList(
                    ...
                    new PiActionParam(
                        SouthConstants.FAR_ID, globalFarIdOf(pdr.sessionId(), pdr.farId())),
                    new PiActionParam(
                        SouthConstants.NEEDS_GTPU_DECAP, pdr.matchesEncapped() ? 1 : 0)))
            .build();
  ...
  @Override
  public FlowRule interfaceToFabricEntry(
      UpfInterface upfInterface, DeviceId deviceId, ApplicationId appId, int priority)
      throws Up4TranslationException {
    ...
    PiCriterion match =
        PiCriterion.builder()
            .matchLpm(
                SouthConstants.HDR_IPV4_DST_ADDR,
          ...
          PiAction.builder()
              ...
              .withParameters(
                  Arrays.asList(
                      ...
                      new PiActionParam(
                          NorthConstants.NOTIFY_FLAG, far.notifies() ? oneByte : zeroByte)))
              .build();
    } else {
      if (far.tunnelSrc() == null
          || far.tunnelDst() == null
          ...
          || far.tunnel().srcPort() == null) {
        throw new Up4TranslationException(
            "Not all action parameters present when translating intermediate encap FAR to logical"
                + " FAR!");
      }
      action =
          PiAction.builder()
              ...
              .withParameters(
                  Arrays.asList(
                      ...
                      new PiActionParam(
                          NorthConstants.NOTIFY_FLAG, far.notifies() ? oneByte : zeroByte),
                      new PiActionParam(
                          NorthConstants.BUFFER_FLAG, far.buffers() ? oneByte : zeroByte),
                      new PiActionParam(
                          NorthConstants.TUNNEL_TYPE_PARAM,
        ...
        PiMatchKey.builder()
            .addFieldMatch(
                new PiExactFieldMatch(
                    NorthConstants.FAR_ID_KEY, ImmutableByteSequence.copyFrom(far.farId())))
    ...
    if (pdr.matchesEncapped()) {
      ...
      matchKey =
          PiMatchKey.builder()
              .addFieldMatch(
                  new PiExactFieldMatch(
                      NorthConstants.SRC_IFACE_KEY, toImmutableByte(NorthConstants.IFACE_ACCESS)))
              .addFieldMatch(
                  new PiTernaryFieldMatch(NorthConstants.TEID_KEY, pdr.teid(), allOnes32))
              .addFieldMatch(
                  new PiTernaryFieldMatch(
                      NorthConstants.TUNNEL_DST_KEY,
                      ImmutableByteSequence.copyFrom(pdr.tunnelDest().toOctets()),
                      allOnes32))
              .build();
    } else {
      decapFlag = 0;
      matchKey =
          PiMatchKey.builder()
              .addFieldMatch(
                  new PiExactFieldMatch(
                      NorthConstants.SRC_IFACE_KEY, toImmutableByte(NorthConstants.IFACE_CORE)))
              .addFieldMatch(
                  new PiTernaryFieldMatch(
                      NorthConstants.UE_ADDR_KEY,
                      ImmutableByteSequence.copyFrom(pdr.ueAddress().toOctets()),
                      allOnes32))
              .build();
    }
  ...
  @Override
  public PiTableEntry interfaceToUp4Entry(UpfInterface upfInterface)
      throws Up4TranslationException {
    int srcIface =
        upfInterface.isAccess() ? NorthConstants.IFACE_ACCESS : NorthConstants.IFACE_CORE;
    int direction =
        upfInterface.isAccess()
            ? NorthConstants.DIRECTION_UPLINK
            : NorthConstants.DIRECTION_DOWNLINK;
    return PiTableEntry.builder()
        ...
        .withAction(
            PiAction.builder()
                ...
                .withParameters(
                    Arrays.asList(
                        new PiActionParam(
                            NorthConstants.SRC_IFACE_PARAM, toImmutableByte(srcIface)),
