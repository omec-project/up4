...
public class FabricUpfProgrammable implements UpfProgrammable {
  ...
  public PdrStats readCounter(int cellId) {
    ...
    List<PiCounterCellHandle> counterCellHandles =
        List.of(
            PiCounterCellHandle.of(
                deviceId,
    ...
    Collection<PiCounterCell> counterEntryResponse =
        client
            .read(DEFAULT_P4_DEVICE_ID, pipeconf)
            .handles(counterCellHandles)
            .submitSync()
            .all(PiCounterCell.class);

    // Process response.
    counterEntryResponse.forEach(
        counterCell -> {
          ...
          if (counterCell
              .cellId()
              .counterId()
              .equals(SouthConstants.FABRIC_INGRESS_SPGW_PDR_COUNTER)) {
            stats.setIngress(counterCell.data().packets(), counterCell.data().bytes());
          } else if (counterCell
              .cellId()
              .counterId()
              .equals(SouthConstants.FABRIC_EGRESS_SPGW_PDR_COUNTER)) {
  ...
  public void addInterface(UpfInterface upfInterface) {
    try {
      FlowRule flowRule =
          up4Translator.interfaceToFabricEntry(upfInterface, deviceId, appId, DEFAULT_PRIORITY);
      log.info("Installing {}", upfInterface);
      flowRuleService.applyFlowRules(flowRule);
      log.debug("Interface added with flowID {}", flowRule.id().value());
    } catch (Up4Translator.Up4TranslationException e) {
      log.warn(
          "Unable to install interface {} on dataplane! Error was: {}",
          upfInterface,
          e.getMessage());
    }
  }
  ...
  public Collection<UpfFlow> getFlows() {
    ...
    // Multiple PDRs can map to the same FAR, so create a one->many mapping of FAR Identifier to
    // flow builder
    ...
    for (FlowRule flowRule : flowRuleService.getFlowEntriesById(appId)) {
      if (up4Translator.isFabricFar(flowRule)) {
        ...
        try {
          ...
          globalFarToSessionBuilder.compute(
    ...
    for (FlowRule flowRule : flowRuleService.getFlowEntriesById(appId)) {
      if (up4Translator.isFabricInterface(flowRule)) {
        try {
          ...
          log.warn(
              "Found what appears to be a interface table entry but we can't translate it?? {}",
              flowRule);
        }
      }
    }
  ...
  public void removeFar(ForwardingActionRule far) {
    ...
    PiCriterion match =
        PiCriterion.builder()
            .matchExact(
                SouthConstants.HDR_FAR_ID,
                up4Translator.globalFarIdOf(far.sessionId(), far.farId()))
            .build();
