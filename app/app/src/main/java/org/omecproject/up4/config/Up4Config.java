...
public class Up4Config extends Config<ApplicationId> {
  ...
  public static final String S1U_PREFIX =
      "s1uPrefix"; // TODO: remove this field after all configs updated
  ...
  public Ip4Address s1uAddress() {
    if (hasField(S1U_ADDR)) {
      ...
      log.warn(
          "UP4 config field {} has been replaced by {}, please update your config!",
          S1U_PREFIX,
          S1U_ADDR);
  ...
  /**
   * Returns the address of the dbuf dataplane interface (in the form of host:port). Or null if not
   * configured. The host part is guaranteed to be a valid IPv4 address.
   *
  ...
  /**
   * Returns the address of the UPF interface that the dbuf device will drain packets towards, or
