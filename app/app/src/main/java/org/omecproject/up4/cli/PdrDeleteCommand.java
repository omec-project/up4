...
public class PdrDeleteCommand extends AbstractShellCommand {
  ...
  @Argument(
      ...
      name = "teid",
      description =
          "Tunnel ID of the tunnel to/from the base station. The presence of this argument implies"
              + " an uplink PDR. An absence implies downlink.",
      required = false,
      multiValued = false)
  int teid = -1;
  ...
  protected void doExecute() {
    ...
    if (teid != -1) {
      pdrBuilder.withTunnelDst(address).withTeid(teid);
    } else {
