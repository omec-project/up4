...
public class Up4NorthComponent {
  ...
  protected void activate() {
    ...
    try {
      server =
          NettyServerBuilder.forPort(AppConstants.GRPC_SERVER_PORT)
  ...
  private void translateEntryAndDelete(PiTableEntry entry) {
    ...
    if (up4Translator.isUp4Interface(entry)) {
      ...
      log.warn(
          "Received unknown table entry for table {} in UP4 delete request:", entry.table().id());
    }
  }
  ...
  private void translateEntryAndInsert(PiTableEntry entry) {
    ...
    if (up4Translator.isUp4Interface(entry)) {
      ...
      log.warn(
          "Received unsupported table entry for table {} in UP4 write request:",
          entry.table().id());
    }
  }

  /**
   * Find all table entries that match the requested entry, and translate them to p4runtime entities
   * for responding to a read request.
   *
  ...
  private List<P4RuntimeOuterClass.Entity> readEntriesAndTranslate(PiTableEntry requestedEntry) {
    ...
    // Respond with all entries for the table of the requested entry, ignoring other requested
    // properties
    ...
    if (up4Translator.isUp4Interface(requestedEntry)) {
      for (UpfInterface iface : up4Service.getUpfProgrammable().getInstalledInterfaces()) {
        ...
        try {
          P4RuntimeOuterClass.Entity responseEntity =
              Codecs.CODECS
                  .entity()
                  .encode(up4Translator.interfaceToUp4Entry(iface), null, pipeconf);
          translatedEntries.add(responseEntity);

        } catch (Up4Translator.Up4TranslationException | CodecException e) {
          log.error(
              "Unable to encode interface to a UP4 read response. Error was: {}", e.getMessage());
        }
      }
      ...
      for (ForwardingActionRule far : up4Service.getUpfProgrammable().getInstalledFars()) {
        ...
        try {
          P4RuntimeOuterClass.Entity responseEntity =
              Codecs.CODECS.entity().encode(up4Translator.farToUp4Entry(far), null, pipeconf);
          translatedEntries.add(responseEntity);
        } catch (Up4Translator.Up4TranslationException | CodecException e) {
          log.error("Unable to encode FAR to a UP4 read response. Error was: {}", e.getMessage());
        }
      }
      ...
      for (PacketDetectionRule pdr : up4Service.getUpfProgrammable().getInstalledPdrs()) {
        ...
        try {
          P4RuntimeOuterClass.Entity responseEntity =
              Codecs.CODECS.entity().encode(up4Translator.pdrToUp4Entry(pdr), null, pipeconf);
          translatedEntries.add(responseEntity);
        } catch (Up4Translator.Up4TranslationException | CodecException e) {
          log.error("Unable to encode PDR to a UP4 read response. Error was: {}", e.getMessage());
        }
      }
    } else {
  ...
  /**
   * Read the requested p4 counter cell, and translate it to a p4runtime entity for responding to a
   * read request.
   *
  ...
  private P4RuntimeOuterClass.Entity readCounterAndTranslate(PiCounterCell requestedCell) {
    ...
    try {
      ...
      log.debug(
          "Encoded response to {} counter read request for counter ID {}.", gress, counterIndex);
      return responseEntity;
    } catch (CodecException e) {
      log.error(
          "Unable to encode counter cell into a p4runtime entity. Exception was: {}",
          e.getMessage());
      return null;
    }
  }
  ...
  public class Up4NorthService extends P4RuntimeGrpc.P4RuntimeImplBase {
    ...
    /**
     * A streamChannel represents a P4Runtime session. This session should persist for the lifetime
     * of a connected controller. The streamChannel is used for master/slave arbitration. Currently
     * this implementation does not track a master, and blindly tells every controller that they are
     * the master as soon as they send an arbitration request. We also do not yet handle anything
     * except arbitration requests.
     *
    ...
    public StreamObserver<P4RuntimeOuterClass.StreamMessageRequest> streamChannel(
      ...
      // streamChannel handles packet I/O and master arbitration. It persists as long as the
      ...
      return new StreamObserver<>() {
        ...
        public void onNext(P4RuntimeOuterClass.StreamMessageRequest value) {
          ...
          if (value.hasArbitration()) {
            log.info(
                "Stream message was arbitration request. "
            ...
            // We currently only respond to arbitration requests. Anything else gets a default
            // response.
    ...
    /**
     * Receives a pipeline config from a client. Discards all but the p4info file and cookie, and
     * compares the received p4info to the already present hardcoded p4info. If the two match, the
     * cookie is stored and a success response is sent. If they do not, the cookie is disarded and
    ...
    public void setForwardingPipelineConfig(
        P4RuntimeOuterClass.SetForwardingPipelineConfigRequest request,
      ...
      if (!otherP4Info.equals(p4Info)) {
        log.warn(
            "Someone attempted to write a p4info file that doesn't match our hardcoded one! What a"
                + " jerk");
      } else {
      ...
      responseObserver.onNext(
          P4RuntimeOuterClass.SetForwardingPipelineConfigResponse.getDefaultInstance());
      responseObserver.onCompleted();
    }
     ...
     */
    @Override
    public void getForwardingPipelineConfig(
        P4RuntimeOuterClass.GetForwardingPipelineConfigRequest request,
          ...
          P4RuntimeOuterClass.GetForwardingPipelineConfigResponse.newBuilder()
              .setConfig(
                  P4RuntimeOuterClass.ForwardingPipelineConfig.newBuilder()
                      .setCookie(
                          P4RuntimeOuterClass.ForwardingPipelineConfig.Cookie.newBuilder()
    ...
    private <E> boolean errorIfSwitchNotReady(StreamObserver<E> responseObserver) {
      if (!up4Service.configIsLoaded()) {
        log.warn(
            "UP4 client attempted to read or write to logical switch before an app config was"
                + " loaded!");
        responseObserver.onError(
            io.grpc.Status.UNAVAILABLE.withDescription("App config not loaded.").asException());
        return true;
      }
      if (!up4Service.upfProgrammableAvailable()) {
        log.warn(
            "UP4 client attempted to read or write to logical switch "
    ...
    @Override
    public void write(
        P4RuntimeOuterClass.WriteRequest request,
    ...
    @Override
    public void read(
        P4RuntimeOuterClass.ReadRequest request,
      ...
      for (P4RuntimeOuterClass.Entity requestEntity : request.getEntitiesList()) {
        ...
        if (requestPiEntity.piEntityType() == PiEntityType.COUNTER_CELL) {
          ...
          if (responseEntity != null) {
            responseObserver.onNext(
                P4RuntimeOuterClass.ReadResponse.newBuilder().addEntities(responseEntity).build());
          } else {
            responseObserver.onNext(P4RuntimeOuterClass.ReadResponse.newBuilder().build());
          }
