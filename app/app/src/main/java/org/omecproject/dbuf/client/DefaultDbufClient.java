...
public final class DefaultDbufClient implements DbufClient {
  ...
  /**
   * Creates a new client for the given gRPC service address and dataplane address (both in the form
   * of host:port), immediately triggering a connection to the gRPC service and Subscribe RPC.
   *
   * <p>The host part of dataplaneAddr must be a valid IPv4 address.
   *
  ...
  static ManagedChannel buildChannel(String serviceAddr) {
    ...
    return NettyChannelBuilder.forAddress(pieces[0], Integer.parseInt(pieces[1]))
  ...
  DefaultDbufClient(String serviceAddr, ManagedChannel channel, String dataplaneAddr) {
    ...
    channel.notifyWhenStateChanged(ConnectivityState.IDLE, this::channelStateCallback);
  ...
  public CompletableFuture<Boolean> drain(Ip4Address ueAddr, Ip4Address dstAddr, int udpPort) {
    ...
    final var request =
        Dbuf.ModifyQueueRequest.newBuilder()
  ...
  void handleNotification(Dbuf.Notification notification) {
    switch (notification.getMessageTypeCase()) {
      case READY:
        ...
        log.info(
            "Dbuf service at {} is READY={} [{}]",
            serviceAddr,
            isReady(),
            TextFormat.shortDebugString(notification.getReady()));
        break;
      case FIRST_BUFFER:
        // TODO: notify PFCP agent (DDN)
        log.info(
            "Received FIRST_BUFFER: {}",
            TextFormat.shortDebugString(notification.getFirstBuffer()));
        break;
      case DROPPED_PACKET:
        // Not sure what to do with this. Drop stats should already be reported to Aether
        // monitoring.
        log.info(
            "Received DROPPED_PACKET: {}",
            TextFormat.shortDebugString(notification.getDroppedPacket()));
        break;
  ...
  private void channelStateCallback() {
    ...
    if (newState != ConnectivityState.SHUTDOWN) {
      ...
      channel.notifyWhenStateChanged(newState, this::channelStateCallback);
    }
  }
  ...
  private void handleRpcError(Throwable throwable, String opDescription) {
    if (throwable instanceof StatusRuntimeException) {
      ...
      log.warn("Error while performing {} on {}: {}", opDescription, serviceAddr, logMsg);
      log.debug("", throwable);
      return;
    }
    log.error(format("Exception while performing %s on %s", opDescription, serviceAddr), throwable);
  }
