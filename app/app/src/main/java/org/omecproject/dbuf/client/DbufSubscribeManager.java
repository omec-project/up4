...
final class DbufSubscribeManager {
  ...
  public void subscribe() {
    synchronized (this) {
      ...
      if (!isCancelled()) {
        log.debug(
            "Cancelling existing subscription for {} before " + "starting a new one",
            client.serviceAddr());
        cancel();
      }
      ...
      if (checkTask == null) {
        checkTask =
            streamCheckerExecutor.scheduleAtFixedRate(
                this::checkSubscription, 0, DEFAULT_RECONNECT_DELAY, TimeUnit.SECONDS);
      }
    }
  }
    ...
    synchronized (this) {
      if (subscribeRequested.get() && isCancelled()) {
        if (client.isServerReachable()) {
          ...
          rpcContext.run(
              () ->
                  DbufServiceGrpc.newStub(client.channel())
                      .subscribe(Dbuf.SubscribeRequest.getDefaultInstance(), responseObserver));
        } else {
          log.debug(
              "Not starting Subscribe RPC for {}, server is NOT reachable", client.serviceAddr());
        }
      }
    }
  }
      ...
      try {
        if (log.isTraceEnabled()) {
          log.trace(
              ...
              client.serviceAddr(),
              TextFormat.shortDebugString(notification));
        }
        ...
        log.error("Exception processing Notification from " + client.serviceAddr(), ex);
      }
    }
    ...
    public void onError(Throwable throwable) {
      if (throwable instanceof StatusRuntimeException) {
        ...
        if (sre.getStatus().getCause() instanceof ConnectException) {
          log.warn("{} is unreachable ({})", client.serviceAddr(), sre.getCause().getMessage());
        } else {
          log.warn(
              "Error on Subscribe RPC for {}: {}", client.serviceAddr(), throwable.getMessage());
        }
      } else {
        log.error(format("Exception on Subscribe RPC for %s", client.serviceAddr()), throwable);
      }
      cancel();
    }
