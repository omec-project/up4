...
public final class UpfFlow {
  ...
  private UpfFlow(
      Type type,
      ImmutableByteSequence pfcpSessionId,
  ...
  public String toString() {
    ...
    return String.format(
        "SEID:%s - %s  -->  %s;\n    >> %s", pfcpSessionId, pdrString, farString, statString);
  }
  ...
  public static class Builder {
    ...
    /**
     * Add a PDR to the session. The PFCP session ID and the FAR ID set by this PDR should match
     * whatever FAR is added (if a FAR is added). If this condition is violated, the call to build()
     * will fail.
     *
    ...
    /**
     * Add a FAR to the session. The PFCP session ID and the FAR ID read by this FAR should match
     * whatever PDR is added (if a PDR is added). If this condition is violated, the call to build()
     * will fail.
     *
    ...
    /**
     * Add a PDR counter statistics instance to this session. The cell id of the provided counter
     * statistics should match the cell id set by the PDR added by setPdr(). If this condition is
     * violated, the call to build() will fail.
     *
    ...
    public UpfFlow build() {
      ...
      if (pdr != null && far != null) {
        checkArgument(
            pdr.sessionId().equals(far.sessionId()), "PFCP session ID of PDR and FAR must match!");
        checkArgument(pdr.farId() == far.farId(), "FAR ID set by PDR and read by FAR must match!");
      }
      if (pdr != null) {
        if (flowStats != null) {
          checkArgument(
              pdr.counterId() == flowStats.getCellId(),
