...
/**
 * A single Forwarding Action Rule (FAR), an entity described in the 3GPP specifications (although
 * that does not mean that this class is 3GPP compliant). An instance of this class will be
 * generated by a logical switch write request to the database-style FAR P4 table, and the resulting
 * instance should contain all the information needed to reproduce that logical switch FAR in the
 * event of a client read request. The instance should also contain sufficient information (or
 * expose the means to retrieve such information) to generate the corresponding fabric.p4 dataplane
 * forwarding state that implements the FAR.
 */
public final class ForwardingActionRule {
  // Match Keys
  private final ImmutableByteSequence
      sessionId; // The PFCP session identifier that created this FAR
  private final int farId; // PFCP session-local identifier for this FAR
  // Action parameters
  private final boolean
      notifyFlag; // Should this FAR notify the control plane when it sees a packet?
  private final boolean dropFlag;
  private final boolean bufferFlag;
  private final GtpTunnel
      tunnel; // The GTP tunnel that this FAR should encapsulate packets with (if downlink)

  private static final int SESSION_ID_BITWIDTH = 96;

  private ForwardingActionRule(
  ...
  /**
   * Return a new instance of this FAR with the action parameters stripped, leaving only the match
   * keys.
   *
   * @return a new FAR with only match keys
   */
  public ForwardingActionRule withoutActionParams() {
    return ForwardingActionRule.builder().setFarId(farId).withSessionId(sessionId).build();
  }
  ...
  public String actionString() {
    ...
    if (dropFlag) {
      ...
      actionParams =
          String.format(
              "Src=%s, SPort=%d, TEID=%s, Dst=%s",
              tunnel.src().toString(),
              tunnel.srcPort(),
              tunnel.teid().toString(),
              tunnel.dst().toString());
    } else {
      actionName = "Forward";
    }
  ...
  /**
   * A description of the tunnel that this FAR will encapsulate packets with, if it is a downlink
   * FAR. If the FAR is uplink, there will be no such tunnel and this method wil return null.
   *
   * @return A GtpTunnel instance containing a tunnel sourceIP, destIP, and GTPU TEID, or null if
   *     the FAR is uplink.
   */
  ...
  public static class Builder {
    ...
    public Builder setTunnel(
        Ip4Address src, Ip4Address dst, ImmutableByteSequence teid, short srcPort) {
      return this.setTunnel(
          GtpTunnel.builder().setSrc(src).setDst(dst).setTeid(teid).setSrcPort(srcPort).build());
    }
