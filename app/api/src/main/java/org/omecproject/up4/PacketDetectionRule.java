...
/**
 * A single Packet Detection Rule (PDR), an entity described in the 3GPP specifications (although
 * that does not mean that this class is 3GPP compliant). An instance of this class will be
 * generated by a logical switch write request to the database-style PDR P4 table, and the resulting
 * instance should contain all the information needed to reproduce that logical switch PDR in the
 ...
 * forwarding state that implements the PDR.
 */
public final class PacketDetectionRule {
  ...
  private final Integer
      farId; // The PFCP session-local ID of the FAR that should apply after this PDR hits
  private final Type type;

  private static final int SESSION_ID_BITWIDTH = 96;

  private PacketDetectionRule(
  ...
  public String toString() {
    ...
    if (hasActionParameters()) {
      actionParams =
          String.format("SEID=%s, FAR=%d, CtrIdx=%d", sessionId.toString(), farId, ctrId);
    }
  ...
  /**
   * Instances created as a result of DELETE write requests will not have action parameters, only
   * match keys. This method should be used to avoid null pointer exceptions in those instances.
   *
  ...
  /**
   * Return a new instance of this PDR with the action parameters stripped, leaving only the match
   * keys.
   *
  ...
  public PacketDetectionRule withoutActionParams() {
    if (matchesEncapped()) {
      return PacketDetectionRule.builder().withTeid(teid).withTunnelDst(tunnelDst).build();
    } else {
      return PacketDetectionRule.builder().withUeAddr(ueAddr).build();
    }
  }
